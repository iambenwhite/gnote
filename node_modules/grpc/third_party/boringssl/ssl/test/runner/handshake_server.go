...
Curves:
	...
	if hs.suite.flags&suitePSK != 0 && hs.sessionState != nil {
		psk = deriveResumptionPSK(hs.suite, hs.sessionState.mainSecret)
		hs.finishedHash.setResumptionContext(deriveResumptionContext(hs.suite, hs.sessionState.mainSecret))
	} else {
	...
	mainSecret := hs.finishedHash.extractKey(handshakeSecret, hs.finishedHash.zeroSecret())
	trafficSecret := hs.finishedHash.deriveSecret(mainSecret, applicationTrafficLabel)
	...
	c.exporterSecret = hs.finishedHash.deriveSecret(mainSecret, exporterLabel)
	c.resumptionSecret = hs.finishedHash.deriveSecret(mainSecret, resumptionLabel)
